<template>
  <div style="background-color:black">
          ...
          <v-carousel-item>
            <v-sheet class="arch" height="100%">
              <v-row class="fill-height" align="center" justify="center">
                <div class="display-3">DIY KITS</div>
              </v-row>
            </v-sheet>
          </v-carousel-item>
        </v-carousel>
    ...
    >WHERE ALL CREATIVITY BEGIN</div>
...
margin-top:20px"
    ...
    <div class="row header-text" style="padding-top:70px">
      ...
      <div
        ...
        @click="showLessDIY = !showLessDIY; getDIYByFilter() "
      >{{showLessDIY===true? "Show All DIY" : "Show Less"}}</div>
    </div>
    <v-sheet
      v-show="showLessDIY"
      class="mx-auto"
    ...
    <div class="row" v-show="showLessDIY==false" style="margin-left:5px; margin-right:5px">
...
export default {
  ...
  data: () => ({
    ...
    category: [
      ...
      { name: "Free DIY", filter: "free", img: "", description: "continue" },
    ],
    hooperSettings: {
      itemsToShow: null,
      centerMode: true,
  ...
  methods: {
    getDIY() {
      serv
        .getRequest(`diy/allDiys/${this.search}`)
        .then((response) => {
    ...
    getDIYByFilter(item) {
      serv
        .getRequest(`books/allDIYcat/${item.filter}`)
        .then((response) => {
          // JSON responses are automatically parsed.
          this.shop = response.data.result;
...
.arch {
  background-image: url("../assets/DIY-carousel-back.jpg");
  filter: gray;
  position: cover;
